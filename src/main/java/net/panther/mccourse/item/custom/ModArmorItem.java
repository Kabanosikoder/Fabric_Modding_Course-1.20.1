...
import net.panther.mccourse.item.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, StatusEffectInstance> MATERIAL_TO_EFFECT_MAP =
            new ImmutableMap.Builder<ArmorMaterial, StatusEffectInstance>()
                    ...
                    .build();
                    // only works with custom armor materials not vanilla armor materials
    ...
    private void evaluateArmorEffects(PlayerEntity player) {
        for(Map.Entry<ArmorMaterial, StatusEffectInstance> entry : MATERIAL_TO_EFFECT_MAP.entrySet()){ // for each loop in Map
    ...
    private void addStatusEffectForMaterial(PlayerEntity player, StatusEffectInstance mapStatusEffect) {
        boolean hasPlayerEffectAlr = player.hasStatusEffect(mapStatusEffect.getEffectType());

        if(!hasPlayerEffectAlr) {
